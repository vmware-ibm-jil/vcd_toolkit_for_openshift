...
cat $OPENSHIFT/install-config-cp4m.yaml.templ| sed -e "s/BASEDOMAIN/$BASEDOMAIN/" -e "s/WORKER/$WORKERNAME/" -e "s/MASTER/$MASTERNAME/" -e "s/PREFIXDOMAIN/$PREFIXTODOMAIN/" -e "s/SERVICENETWORK/$SERVICENETWORK\/$SERVICENETWORKCIDR/" > $IGNITIONDIR/install-config.yaml
